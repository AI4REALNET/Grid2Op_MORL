...
class PPO(tf.Module):
    ...
    def train(self, obs, returns, actions, values, neg_log_p_old, advs, lr=3e-4, clip_range=0.2):
        grads, policy_loss, value_loss, entropy, approx_kl, clip_ratio = self._get_grad(
            obs, returns, actions, values, neg_log_p_old, advs, clip_range
        )

        self.optimizer.learning_rate = lr

        # Filter out None gradients – Keras 3 will crash if the list is empty.
        ...
        if not grads_and_vars:
            # Debug info: avoids crashing the whole run if a batch has no valid grads.
            print("[PPO.train] All gradients are None for this batch – skipping optimizer step.")
    ...
    def _get_grad(self, obs, returns, actions, values, neg_log_p_old, advs, clip_range):
        ...
        neg_log_p_old = tf.cast(neg_log_p_old, tf.float32)
        ...
        var_list = self.model.model.trainable_variables  # PVNet variables
...
class Policy_Value_Network(tf.Module):
    def __init__(self, junior_path='./JuniorModel.keras'):
        super().__init__()  # IMPORTANT: enable tf.Module tracking

        # warm start from Junior Student
        self.model = PVNet()

        # build PVNet
        dummy_input = tf.zeros((1, 1221), dtype=tf.float32)
        _ = self.model(dummy_input)

        # copy parameters from Junior
        self._params_copy(junior_path)

    def _params_copy(self, path):
        # path should be a .keras or .h5 file; for us it's ./JuniorModel.keras
        model = tf.keras.models.load_model(path, compile=False)

        # Copy shared layers: keep the original mapping from the repo
