...
if __name__ == '__main__':
    ...
    if task == 'Train':
        ...
        ckpt_dir = 'ckpt/cp-{epoch:04d}.weights.h5'
        ...
        # Use the weights file produced during training (e.g., epoch 50)
        save_path = './ckpt/cp-0050.weights.h5'  # adjust epoch if needed

        # 1) Build the model by calling it once on dummy data
        dummy_input = np.zeros((1, s_train.shape[-1]), dtype=np.float32)
        _ = junior.model(dummy_input, training=False)  # or junior.model.predict(dummy_input)

        # 2) Now it's built, we can safely load the weights
        junior.model.load_weights(save_path)

        # 3) Save full model in a Keras 3–compatible format
        junior.model.save('../SeniorStudent/JuniorModel.keras')
