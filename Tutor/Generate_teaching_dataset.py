...
if __name__ == '__main__':
    ...
    # Determine observation vector length dynamically from the current environment
    sample_obs = env.get_obs()
    obs_vect = sample_obs.to_vect()
    obs_dim = obs_vect.size
    print(f"[Tutor dataset] Detected obs_dim = {obs_dim}, total feature + label dim = {obs_dim + 1}")

    # first col for label, remaining obs_dim cols for feature (observation.to_vect())
    # start with an empty array of shape (0, obs_dim + 1)
    records = np.zeros((0, obs_dim + 1), dtype=np.float32)
